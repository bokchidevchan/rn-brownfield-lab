...
Hermes 의 시작 시간 이점은 저사양 실기기에서 다시 재야 합니다. `[미검증]`
크기 차이 3.3MB 는 확실합니다.
...
보완이 필요한 부분입니다. 위 숫자는 전부 에뮬레이터이고 호스트가 Apple Silicon 이라
낙관적입니다. 3회 측정은 편차를 걸러내지 못합니다. TTI 를 "첫 렌더"로 볼지
"사용자가 만질 수 있는 시점"으로 볼지도 정해야 합니다. `[미검증]`
