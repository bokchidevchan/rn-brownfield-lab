 ...
 * Stackflow 초기화.
 *
 * 세 개의 층이 나뉘어 있는 게 이 라이브러리의 특징입니다.
 *   core          스택 상태 계산만 합니다. UI 를 모릅니다.
 *   renderer      상태를 화면에 어떻게 그릴지
 *   basicUIPlugin 실제 화면 전환 애니메이션, 헤더, 스와이프백
 *
 * historySyncPlugin 을 넣으면 스택이 브라우저 History API 와 동기화됩니다.
 * 웹뷰 안에서 이게 중요한 이유는 안드로이드 백 버튼 때문입니다.
 * 백 버튼이 웹뷰 히스토리를 되돌리도록 네이티브가 연결해 주면,
 * 그 히스토리가 곧 Stackflow 의 스택이 됩니다.
 */
...
export const { Stack } = stackflow({
  ...
  plugins: [
    ...
    historySyncPlugin({
      routes: {
        ProductList: '/stackflow/',
        ProductDetail: '/stackflow/products/:productId',
        ReviewList: '/stackflow/products/:productId/reviews',
      },
