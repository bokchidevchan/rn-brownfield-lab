# 11. 메모리: 네이티브, RN, Lynx, 웹뷰는 각각 어디에 메모리를 쓰나

같은 에뮬레이터에서 네 방식의 화면을 하나씩 띄우고 `dumpsys meminfo` 로 프로세스 메모리(PSS)를 쟀습니다. 03번(웹뷰 vs RN 진입 시간), 10번(Lynx vs RN 번들)에 이어, 이번에는 "떠 있는 동안 무엇이 메모리를 차지하는가"를 봅니다.

네이티브 판은 이 실험을 위해 03번 bench 앱에 추가했습니다(`NativeDetailActivity`). 웹뷰, RN 판과 같은 화면 구성(섹션 4, 행 6, 버튼 2)이라 "같은 화면을 그리는 데 각 방식이 얼마를 내는가"가 직접 비교됩니다.

## 먼저 구조부터: 넷은 프로세스 모양이 다릅니다

숫자보다 이게 본론입니다. 어디에 메모리가 잡히는지가 방식마다 다릅니다.

**네이티브 화면은 증분이 거의 없습니다.** 화면의 실체가 뷰 객체(자바 힙)와 렌더링 버퍼뿐이라, 이미 떠 있는 앱 프로세스에 한 화면 얹는 비용은 MB 한 자릿수 초반입니다. 별도 런타임을 부팅하지 않는다는 것이 메모리에서는 이렇게 나타납니다. 나머지 세 방식의 숫자는 전부 "이 기준에서 얼마나 더 내는가"로 읽으면 됩니다.
...
Pixel 9 에뮬레이터(API 36, arm64). 네이티브, 웹뷰, RN 은 03번 bench 앱(release, RN 0.76.9 Old Arch + Hermes)의 같은 상품 상세 화면이고, Lynx 는 LynxExplorer 4.0.1 에 10번 카운터 번들(프로덕션 빌드, 정적 서버 서빙)을 열었습니다. 각 시나리오는 콜드 시작 후 5초 간격 3회 측정, 마지막(안정) 값입니다. bench 네 줄은 같은 APK, 같은 세션에서 연속으로 쟀습니다.

| 시나리오 | PSS | 홈 대비 |
|---|---|---|
| bench 홈 (기준) | 59.2 MB | - |
| 네이티브 상세 화면 | 61.4 MB | +2.1 MB |
| RN 화면 | 78.1 MB | +18.9 MB |
| 웹뷰 화면 (앱 프로세스) | 118.1 MB | +58.9 MB |
| 웹뷰 샌드박스 렌더러 (별도 프로세스) | 59.8 MB | +59.8 MB |
| **웹뷰 합계** | **177.9 MB** | **+118.7 MB** |
| LynxExplorer 홈 | 68.8 MB | 기준 (셸) |
| LynxExplorer + 카운터 카드 | 78.8 MB | 카드 +10.0 MB |

같은 화면 하나를 띄우는 데 네이티브 약 2MB, RN 약 19MB, 웹뷰 약 119MB 를 냈습니다. 자릿수가 하나씩 다릅니다. Lynx 는 셸 위에 카드 하나가 약 10MB 였는데, 화면이 더 단순해서 아래 주의 절의 조건이 붙습니다.

## 내역에서 보이는 것

`dumpsys meminfo` 의 항목별 내역에서 구조가 그대로 드러납니다.

**네이티브 (홈 대비):** Native Heap +0.2MB, 자바 힙 +0.2MB. 늘어난 것이 사실상 뷰 객체와 렌더링 버퍼뿐입니다.

**웹뷰 (앱 프로세스, 홈 대비):** `.apk mmap` 약 +20MB 와 `.so mmap` 약 +8MB 는 OS 의 WebView APK 와 엔진 코드가 앱 주소 공간에 매핑된 것입니다. Native Heap +9.1MB 와 Unknown 증가분은 브라우저 쪽 할당자입니다. 여기에 렌더러 프로세스 59.8MB 가 통째로 더해집니다. 페이지 하나(로컬 정적 페이지)치고 큰데, 이게 브라우저 한 탭의 기본 무게입니다.

**RN (홈 대비):** Native Heap +2.9MB 에 Hermes JS 힙이 들어 있고, Unknown(번들 매핑 등), `.apk mmap`(앱 안의 리소스 매핑), Dalvik +2.9MB(네이티브 뷰의 자바 객체)로 고르게 늘었습니다. 별도 프로세스가 없습니다.
...
# 베이스라인 / 네이티브 / 웹뷰 / RN (bench release 설치 전제. 03번 참고)
...
adb shell am start -n com.example.bench/com.example.bench.MainActivity --es auto native
adb shell dumpsys meminfo com.example.bench -s
adb shell am force-stop com.example.bench
...
- 에뮬레이터 1회 측정(시나리오당 3회 읽기 중 안정값)입니다. 실행마다 5~10MB 흔들리는 것을 확인했습니다. 다른 세션의 측정에서는 홈 54.5MB, 웹뷰 합계 154.3MB(렌더러 51.1MB)가 나오기도 했습니다. 절대값보다 자릿수와 구조를 봐야 합니다.
- 화면 내용이 4자 동일하지 않습니다. 네이티브/웹뷰/RN 은 같은 화면이라 직접 비교가 되지만, Lynx 는 더 단순한 카운터 화면입니다.
