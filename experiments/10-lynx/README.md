...
## 엔진 층: PrimJS 와 렌더링 방식

Lynx 를 "또 하나의 RN" 이 아니라 별개 프레임워크로 만드는 것은 엔진 층입니다.

**PrimJS 는 QuickJS 를 개조한 엔진입니다.** ByteDance 가 Lynx 전용으로 만들었고 ES2019 를 지원합니다. QuickJS 와 다른 점으로 저장소가 세 가지를 듭니다. 스택 캐싱과 레지스터 최적화를 넣은 템플릿 인터프리터, 참조 카운팅 대신 GC 를 쓰는 메모리 관리, 그리고 Lynx 객체 모델과 직접 맞물리는 오브젝트 모델입니다. Octane 벤치마크에서 QuickJS 대비 약 28% 높다고 밝히고 있습니다(3735 대 2904).

RN 의 Hermes 와 역할이 같아 보이지만 위치가 다릅니다. Hermes 는 RN 의 JS 스레드에서 앱 코드 전체를 돌립니다. PrimJS 는 Lynx 의 메인 스레드 런타임에 상주해서 첫 렌더와 우선순위 높은 이벤트만 맡고, 일반 앱 코드는 백그라운드 런타임에서 돕니다.

**화면은 네이티브 뷰로 그립니다.** 이 부분이 Flutter 와 갈리는 지점이라 확인했습니다. Lynx 문서는 "rendered natively by the Lynx Engine into the UI primitives for each platforms" 라고 적고 있고, `view` 는 Android 의 `ViewGroup` 과 iOS 의 `UIView`, `text` 는 `TextView` 와 `UITextView`, `image` 는 `ImageView` 와 `UIImageView` 가 됩니다. Flutter 는 Skia 나 Impeller 로 픽셀을 직접 그려서 OS 의 UI 시스템을 쓰지 않는데, Lynx 는 RN 과 마찬가지로 플랫폼 뷰에 매핑합니다.

정리하면 Lynx 는 렌더링 철학에서 Flutter 보다 RN 에 가깝고, 스레드 구조와 JS 엔진에서 RN 과 갈립니다.

## RN 뉴 아키텍처와 무엇이 같고 다른가

04 번에서 다룬 RN 뉴 아키텍처와 방향이 겹칩니다. 둘 다 JSON 직렬화 브리지를 없애고 JS 와 네이티브가 직접 통하게 만들었고, 둘 다 빌드 타임에 바이트코드를 만들어 시작 시간을 줄입니다(Hermes 의 HBC, PrimJS 의 QuickJS 계열 바이트코드).

다른 것은 스레드를 나누는 방식입니다. RN 은 JS 스레드 하나가 기본이고, UI 스레드로 일을 보내려면 Reanimated 의 worklet 처럼 별도 장치를 씁니다. Lynx 는 프레임워크가 처음부터 둘로 갈라 놓고, 앱 코드는 기본이 백그라운드 런타임이며 메인 스레드로 보내려면 표시를 해야 합니다. 이 저장소의 10 번 예제에서 `'background only'` 지시어가 그 표시입니다.

인터넷에 도는 비교 글 중에 사실과 다른 것이 섞여 있어 확인한 것을 적어 둡니다.

- "Lynx 엔진은 Rust 로 만들었다"는 틀립니다. lynx 저장소의 언어 구성은 C++ 3,500만 줄에 Java, Objective-C, Kotlin, Swift 이고 Rust 는 없습니다. Rust 인 것은 빌드 도구 쪽(Rspack 기반의 Rspeedy)입니다. PrimJS 도 C++ 와 어셈블리입니다.
- "Lynx 는 가상 DOM 을 건너뛴다"도 틀립니다. ReactLynx 의 의존성은 Preact 포크(`@lynx-js/internal-preact`)이고 Preact 는 가상 DOM 을 씁니다. Lynx 가 줄인 것은 가상 DOM 자체가 아니라 그 결과를 네이티브로 넘기는 경로입니다.
- "비즈니스 로직이 아무리 무거워도 UI 는 무조건 60fps 를 유지한다"는 과장입니다. 백그라운드에서 계산한 결과를 화면에 반영하는 일은 결국 메인 스레드가 합니다. Lynx 문서도 첫 프레임을 보장하려고 "briefly blocking the main thread until the first frame is fully rendered" 한다고 적고 있습니다. 메인 스레드가 성역인 것이 아니라, 무거운 일을 기본적으로 그쪽에 안 올리는 구조인 것입니다.

## 그럼 RN 을 튜닝하면 같아지나

상당 부분 됩니다. Reanimated 의 worklet 은 UI 스레드에 별도 JS 컨텍스트를 만들어 제스처와 애니메이션을 JS 스레드 없이 처리하고, 무거운 연산은 별도 워커 스레드나 JSI 기반 C++ 모듈로 내보낼 수 있습니다. 목적지는 비슷해집니다.

차이는 성능의 상한이 아니라 기본값입니다. RN 에서는 스레드를 나누는 게 개발자의 선택이고, 나누는 순간 worklet 용 코드와 일반 코드를 갈라 쓰고 값 공유를 신경 써야 합니다. 팀에서 그 규율을 유지하는 게 실제 비용입니다. Lynx 는 나뉜 상태가 기본이고 합치려면 표시를 해야 합니다.

기존 앱에 화면 몇 개를 붙이는 상황이라면 이 차이가 결정적이지 않습니다. 01~07 번에서 실제로 시간을 먹은 것은 스레드 모델이 아니라 라이브러리 충돌, 버전별 진입점 변화, 배포 파이프라인이었습니다.
...
| JS 엔진 | 웹뷰의 것 | PrimJS (QuickJS 개조, ES2019) | Hermes (기본) |
...
- 이중 스레드가 실제로 프레임 드랍을 얼마나 막아 주는지는 재지 않았습니다. 구조 설명과 공개 자료 확인까지입니다. 무거운 로직을 돌리면서 스크롤 프레임을 재는 실험을 하려면 RN 과 Lynx 양쪽에 같은 부하를 거는 harness 가 따로 필요합니다.
